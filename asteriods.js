asteroid=function(pos,r){
	...
	this.path=[];
	this.r=r
	this.vel=this.pos.add(new vec2(-width/2,-height/2)).neg().norm().rotate((Math.random()-0.5)*0.25)
...
asteroid.prototype={
	"update":function(){
		...
		if(this.pos.add(new vec2(width/2,height/2)).mag()>Math.max(height,width)*1.5){
			this.destroyed=true;
		}
	},
	"show":function(){
...
game={
	...
	"update":function(){
		...
		this.projectiles.forEach(x=>{
			if(x.pos.add(new vec2(width/2,height/2)).mag()>Math.max(height,width)*1.5){
				x.destroyed=true;
			}
			if(!x.destroyed){
			proj.push(x)
		...
		if(this.asteroids.length<10){
			...
			game.asteroids.push(new asteroid(v,(Math.random()-0.5)*10+width/10))
		}
		this.projectiles.forEach(x=>{
